...
DATA WORK.STREAM_LIST (KEEP = _name _code Ref_e Ref_q Ref_node Ref_incl Ref_peri);
	SET WORK.STREAM_LIST (rename = (_e = Ref_e _q = Ref_q _node = Ref_node _incl = Ref_incl _peri = Ref_peri)) 
	NOBS = CANDIDATES;
...
DROP _code _name Ref_e Ref_q Ref_node Ref_incl Ref_peri;

ARRAY shower_name_list(&CANDIDATES.) $50 _TEMPORARY_;
ARRAY shower_code_list(&CANDIDATES.) $3 _TEMPORARY_;
...
IF _N_ = 1 THEN DO;
	DO I = 1 TO &Candidates.;
		...
		shower_name_list(I) = _name;
		shower_code_list(I) = _code;
		node_list(I) = %radians(Ref_node);
		peri_list(I) = %radians(Ref_peri);
		incl_list(I) = %radians(Ref_incl);
		e_list(I) = Ref_e;
		q_list(I) = Ref_q;
